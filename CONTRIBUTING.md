...
- 功能开发应拆分为可独立理解、验证和回退的工作单元；不要按文件数量或开发时长机械拆分。
- 每个提交必须保持仓库处于可构建状态，并同时包含该单元必需的实现、测试、生成产物、契约或迁移。
- Codex 执行功能开发时，每个工作单元通过适用检查后自动创建本地提交，无需再次确认。
- 提交前必须检查工作区和暂存区、运行 GitNexus 变更检测，并只暂存当前单元拥有的文件或 hunk。
- 不得提交测试失败、已知不完整或混入其他任务和用户现有修改的工作单元。
- 自动提交授权不包含 push、merge、cherry-pick、rebase、tag、发布或删除分支；这些操作仍需明确授权。
- 最终交付应列出有序 commit 范围，并说明每个 commit 对应的工作单元和验证结果。
